...
my ($masterMetaFile, $rawRegionsFile, $outSfldMapFile, $outNetInfoFile, $outRegionsFile, $outJobIds, $outDicedJobIds, $outAscore, $rawDicedIdFile, $jobIdFile);
my $result = GetOptions(
    "master-job-file=s"     => \$masterMetaFile, # raw mapping file from excel: cluster_id<TAB>name_including_SFLD_num
...
die "Need --raw-map" if not $masterMetaFile or not -f $masterMetaFile;
...
#die "Need --regions and --out-regions" if $rawRegionsFile and not (-f $rawRegionsFile or $outRegionsFile);
#die "--out-sfld-map must exist" if $outSfldMapFile and not -f not $outSfldMapFile;
...
#my %dicedInfo;
#if ($rawDicedIdFile and -f $rawDicedIdFile) {
#    %dicedInfo = getDicedInfo($rawDicedIdFile);
#}
...
foreach my $clusterId (keys %$masterData) {
    if (scalar @{$masterData->{$clusterId}->{children}}) {
        push @nonDicedJobOut, {}; #TODO
    } else {
        push @dicedJobOut, {}; #TODO
    }
}

# If it has children don't dice it.  It's a parent if clusters exist that share it's name (e.g. Cluster-N is a parent because Cluster-N-X exists)
...
saveNetInfo(\%clusterNames);

saveIdFiles(\@nonDicedJobOut, \@dicedJobOut);

if ($rawRegionsFile and -f $rawRegionsFile and $outRegionsFile) {
    processRegionsFile(\%clusterNames);
}
...
sub getDicedInfo {
    ...
    while (<$dicedFh>) {
        ...
        $ascore = formatAscore($ascore);
        push @{ $info{$cluster} }, {ascore => $ascore, cluster_job => $clusterJob, cr_job => $crJob};
    }
...
sub saveNetInfo {
    ...
    my $sfldMapFh;
    open $sfldMapFh, ">", $outSfldMapFile if $sfldMapFile and $outSfldMapFile;
    ...
    close $sfldMapFh if $sfldMapFh;
    close $netInfoFh;
}

sub saveIdFiles {
    my $nonDicedJobOut = shift;
    my $dicedJobOut = shift;
